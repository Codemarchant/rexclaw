...
| 🧍 **生きている 3D アバター** | three.js + @pixiv/three-vrm：ライブ音声からのビセームリップシンク、待機中の呼吸・まばたき・眼球サッケード、カメラ目線、そして会話の流れに応じてモデル自身がトリガーする感情表現とボディランゲージ。 |
...
`REXCLAW_PORT` でポートを変更、`REXCLAW_NO_BROWSER=1` でブラウザの自動起動を
抑制、`REXCLAW_DATA_DIR` でデータフォルダを移動できます。スクリプトが自動化
しているのは以下だけです：
...
5 人とも **設定 → Companions** で編集できます — プロンプト、ボイス、
...
**設定 → Companions → Edit → Remote MCP connections**。各接続には、サーバー
