...
| 🖥️ **デスクトップマスコットモード** | デスクトップアプリでアバターをウィンドウから「ポップアウト」：フレームレスで透明な小さい最前面オーバーレイとして、作業中もコンパニオンがデスクトップに浮かびます — 通話もそのまま。キャラクター（またはハンドル）をドラッグで移動、ピン留めの切替、サイズ変更、トレイアイコンから四隅へスナップ、ワンクリックでアプリに戻る — 会話はハンドオフをまたいで再開されます。さらに**ゴーストモード**では、クリックがウィンドウを素通りし、アバターはカーソルを避けてフェード（ピクセル単位の当たり判定）— 作業の上に立っていても邪魔になりません。 |
