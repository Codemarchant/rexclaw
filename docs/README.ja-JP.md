...
| 🎨 **Grok Imagine 内蔵** | 「模様替えして」と頼めば `change_background` がライブシーンを差し替え。トランスクリプトへの画像生成、チャットにアップロードした写真の編集に加え、Imagine ライブラリの画像 — セルフィー・アップロード・過去の生成物 — をリミックスして新しい画像を作ること(`create_image` の `source_images`)も。 |
