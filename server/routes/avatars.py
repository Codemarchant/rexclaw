...
def manage_list(payload: dict = Body(default={}), con=Depends(db_con)):
    ...
    for r in rows:
        ...
        d["fullbody_url"] = portraits.fullbody_url(r["vrm_path"])
        out.append(d)
    return out
...
@router.get("/fullbody")
def fullbody(vrm: str, v: str = ""):
    """The full-body portrait PNG sidecar for a served VRM path, as
    generated by the avatar editor. `v` is a cache-buster only."""
    path = portraits.fullbody_file(vrm)
    if not path:
        raise UserError("No full-body portrait for that avatar.")
...
def set_portrait(payload: dict = Body(default={}), con=Depends(db_con)):
    ...
    every file in the folder, so it travels with the avatar. `kind` picks
    the face portrait (default) or the full-body one. Returns the fresh
    portrait_url and fullbody_url for that VRM."""
    pack_key = payload.get("pack_key")
    filename = payload.get("filename") or ""
    kind = payload.get("kind") or "face"
    if kind not in ("face", "fullbody"):
        raise UserError("kind must be 'face' or 'fullbody'.")
    ...
    side = portraits.fullbody_sidecar_path(vrm) if kind == "fullbody" else portraits.sidecar_path(vrm)
    tmp = side.with_suffix(".tmp")
    tmp.write_bytes(png)
    tmp.replace(side)
    web_path = f"/avatars/{pack_key}/{filename}"
    return {"ok": True,
            "portrait_url": portraits.portrait_url(web_path),
            "fullbody_url": portraits.fullbody_url(web_path)}
...
def get(payload: dict = Body(default={}), con=Depends(db_con)):
    ...
    # Per-outfit portraits keyed by the manifest's vrm ref (pack filename or
    # shared /assets path) — same pipeline as the main portrait.
    outfit_portraits = {}
    for o in manifest.get("outfits") or []:
        ref = o.get("vrm")
        if not ref:
            continue
        web_path = ref if ref.startswith("/") else f"/avatars/{pack_key}/{ref}"
        outfit_portraits[ref] = {"face": portraits.portrait_url(web_path),
                                 "fullbody": portraits.fullbody_url(web_path)}
    return {"pack_key": pack_key, "manifest": manifest,
            "files": avatar_packs.list_pack_files(pack_key),
            "outfit_portraits": outfit_portraits}
