...
this app". Tool policy deliberately does NOT live here: app-level tool
habits are injected centrally by session_service's preamble/postamble
(gated on each companion's enabled tools), so prompts carry personality
only and central tuning reaches every companion — including user-created
ones. Review and tune freely — these are starting points, not canon.
